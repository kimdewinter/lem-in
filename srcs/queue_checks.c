...
/*   Updated: 2020/07/21 16:51:25 by lravier       ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
// 			&& tmp->neighbours[i]->start_dst < tmp->start_dst
...
// 	{
// 		add_to_bitfield(prev, tmp->unavailable);
// 		tmp->is_conj = 0;
// 		i = 0;
// 		found = 0;
// 		while (i < tmp->neighbours_len)
...
// 			&& room_in_bitfield(tmp->neighbours[i], tmp->unavailable) == 0)
// 			{
// 				prev = tmp;
// 				tmp = tmp->neighbours[i];
// 				found = 1;
...
// 	&& item->path->len <= round - 1)
// 		return (0);
// 	if ((dst->weight == item->dst->weight) && dst->weight != 0)
// 	{
// 		if (has_overlap(item->path->bitconj, dst->bitconj, map) == 1
// 		|| (opens_options(item->dst->unavailable, dst->unavailable, map) == 0
