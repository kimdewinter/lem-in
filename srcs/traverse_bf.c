...
/*   Updated: 2020/08/25 15:26:38 by kim           ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
								...
								const size_t call_code)
{
	...
	{
		if (room_in_bitfield(room->neighbours[i], in_paths) != 1 &&
			((call_code == LVL_GRPH_E2S &&
										...
										const t_map *map)
{
	while (queue->head != NULL)
	{
		...
		{
			if (add_nbs_to_q(
				queue, in_paths, queue->head->room, call_code) == EXIT_FAILURE)
				return (EXIT_FAILURE);
		}
								...
								const t_map *map)
{
	...
	if (add_nbs_to_q(&queue, in_paths, room_to_begin_from, call_code) == EXIT_FAILURE)
		return (EXIT_FAILURE);
	if (queue.head != NULL)
	{
		if (exec_traverse_bf(&queue, in_paths, call_code, map) == EXIT_FAILURE)
			return (EXIT_FAILURE);
	}
	return (EXIT_SUCCESS);
}
