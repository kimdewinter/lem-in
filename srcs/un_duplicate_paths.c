...
t_connection **conn, int *changed, t_map *map)
{
	...
	&& room_in_bitfield((*conn)->dst->neighbours[paths->j],
	(*conn)->dst->unavailable) == 0)
	{
...
int *changed)
{
	...
	paths.i = 0;
	paths.j = 0;
	while ((size_t)paths.i < (*conn)->dst->neighbours_len)
	{
		setup_conn(&paths.curr, (*conn)->dst);
		if ((*conn)->dst->neighbours[paths.i] != (*conn)->dst_nb
		&& (*conn)->dst->neighbours[paths.i] != (*conn)->src_nb
		&& room_in_bitfield((*conn)->dst->neighbours[paths.i],
		(*conn)->dst->unavailable) == 0)
		{
			...
			{
				paths.j = 0;
				while ((size_t)paths.j < (*conn)->dst->neighbours_len)
				{
