...
/*   Updated: 2020/06/04 19:16:34 by lravier       ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
size_t		count_active(t_routes_wrapper *rw)
{
	size_t	i;
	size_t	active;
...
t_route			*copy_route(t_route *route, t_subpath *pt, t_map *map)
{
	...
	if (new)
	{
		...
		if (copy_bitconj(&(new->bitconj), route->bitconj, map) == EXIT_SUCCESS)
		{
			...
			while (i < route->len)
			{
				new->route[i] = route->route[i];
				i++;
			}
			i = 0;
			while (i < pt->len)
			{
				new->route[i + route->len] = pt->path[i];
				i++;
			}
			return (new);
		}
...
size_t		add_nbs_to_routes(t_routes_wrapper *rw, t_route *route, t_map *map)
{
	...
	if (add_subpath_to_route(route, add_to_orig, map) == EXIT_SUCCESS)
		return (EXIT_SUCCESS);
	return (EXIT_FAILURE);
}

size_t		solve_paths(t_routes_wrapper *rw, t_map *map)
{
	size_t	active;
	size_t	i;

	active = count_active(rw);
	while (active != 0)
	{
		i = 0;
		while (i < rw->num_paths)
		{
			...
			{
				if (add_nbs_to_routes(rw, rw->routes[i], map) == EXIT_FAILURE)
					return (EXIT_FAILURE);
			}
			else
				rw->routes[i]->end = 1;
...
size_t		build_paths(t_map *map)
{
	...
	&& setup_starting_paths(rw, map) == EXIT_SUCCESS)
	{
		solve_paths(rw, map);
		print_troute(rw);
		return (EXIT_SUCCESS);
